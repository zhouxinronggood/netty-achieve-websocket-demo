...
<script>
// vue3中没有this, 这个proxy就相当于this
import { getCurrentInstance, onMounted, ref, reactive, watch, inject} from 'vue';
...
//export default {
  //setup(){
    ...
    console.log(proxy.$);
    proxy.a.get("http://10.31.88.155:19998:/user/online/list")
          .then(({data}) => {
            console.log(response)
            //this.onlineUserList = data.data
            //不显示当前UserId，只显示别的在线的UserId
            //for (var i = 0; i < this.onlineUserList.length; i++) {ßßßß
              // debugger
  ...
  const created = () => {
    ...
    this.webSocket = new WebSocket("ws://10.31.88.155:19998/websocket")
    this.initWebSocket()
    //随机获取一个UserId
  ...
  methods: {
    ...
    getOnlineUserList() {
      ...
      request.get("http://10.31.88.155:19998:/user/online/list")
          .then(({data}) => {
            this.onlineUserList = data.data
            //不显示当前UserId，只显示别的在线的UserId
